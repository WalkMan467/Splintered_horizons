...
$execute \
    if entity @a[distance=..$(RequiredPlayerRange),tag=st_holding_ticking] run tag @s add st_player_near

#Rotatation+color
scoreboard players add @s temp 0

execute \
    if score @s temp matches 1.. run scoreboard players remove @s temp 1

execute \
    if score @s temp matches 0 at @s run rotate @s ~10 ~

execute \
    if score @s temp matches 1 at @s run rotate @s ~30 ~
team join spawner_tweaker_red @s

execute \
    if entity @s[tag=st_player_near] run team join spawner_tweaker_green @s
...
execute \
    ...
    as @e[limit=1,sort=nearest,distance=..1,tag=st_box] run data modify entity @s block_state.Name set value "minecraft:gray_stained_glass"
...
execute \
    as @e[distance=..0.1,tag=st_radius,tag=!st_radius_y] run function spawner_tweaker:spawner_tweaking/radius with storage spawner_tweaker:temp temp

execute \
    as @e[distance=..0.1,tag=st_radius_y] run function spawner_tweaker:spawner_tweaking/radius with storage spawner_tweaker:temp temp


execute \
    if score first_ticking temp matches 1.. \
    as @e[distance=..0.1,tag=st_radius] run data merge entity @s {text:{"text":"⭕","color":"#ee7171","italic":false},text_opacity:80}

execute \
    if score first_ticking temp matches 1.. \
    if entity @s[tag=st_player_near] \
    as @e[distance=..0.1,tag=st_radius] run data merge entity @s {text:{"text":"⭕","color":"#58eb71","italic":false},text_opacity:50}

execute \
    unless score first_ticking temp matches 1.. \
    as @e[distance=..0.1,tag=st_radius] run data merge entity @s {text:{"text":"⭕","color":"red","italic":false},text_opacity:100}

execute \
    unless score first_ticking temp matches 1.. \
    if entity @s[tag=st_player_near] \
    as @e[distance=..0.1,tag=st_radius] run data merge entity @s {text:{"text":"⭕","color":"dark_green","italic":false},text_opacity:60}

#Adjust glass marker
#execute \
    if entity @s[tag=st_player_near] run data merge entity @s {item:{id:"minecraft:green_stained_glass",count:1b}}
#execute \
    if entity @s[tag=!st_player_near] run data merge entity @s {item:{id:"minecraft:red_stained_glass",count:1b}}

#End here \
    if title has already been shown

execute \
    if score first_ticking temp matches 1.. run return 1
...
#Adjust color of titles \
    as needed

execute \
    if score setting temp matches 1 run data modify storage spawner_tweaker:temp variables.spawn_count_title set value "light_purple"

execute \
    if score setting temp matches 1 run data modify storage spawner_tweaker:temp variables.spawn_count_color set value "green"

execute \
    if score setting temp matches 2 run data modify storage spawner_tweaker:temp variables.max_nearby_title set value "light_purple"

execute \
    if score setting temp matches 2 run data modify storage spawner_tweaker:temp variables.max_nearby_color set value "green"

execute \
    if score setting temp matches 3 run data modify storage spawner_tweaker:temp variables.range_title set value "light_purple"

execute \
    if score setting temp matches 3 run data modify storage spawner_tweaker:temp variables.range_color set value "green"

execute \
    if score setting temp matches 4 run data modify storage spawner_tweaker:temp variables.player_range_title set value "light_purple"

execute \
    if score setting temp matches 4 run data modify storage spawner_tweaker:temp variables.player_range_color set value "green"

execute \
    if score setting temp matches 5 run data modify storage spawner_tweaker:temp variables.delay_title set value "light_purple"

execute \
    if score setting temp matches 5 run data modify storage spawner_tweaker:temp variables.delay_color set value "green"

execute \
    if score setting temp matches 6 run data modify storage spawner_tweaker:temp variables.block_light_title set value "light_purple"

execute \
    if score setting temp matches 6 run data modify storage spawner_tweaker:temp variables.block_light_color set value "green"

execute \
    if score setting temp matches 7 run data modify storage spawner_tweaker:temp variables.sky_light_title set value "light_purple"

execute \
    if score setting temp matches 7 run data modify storage spawner_tweaker:temp variables.sky_light_color set value "green"

#Run titles

execute \
    as @a[limit=1,sort=nearest,tag=st_holding_ticking] run function spawner_tweaker:spawner_tweaking/hotbar_display with storage spawner_tweaker:temp variables
