...
execute \
    if data storage spawner_tweaker:temp potentials.potentials[0].data.entity.Passengers[] run data modify storage spawner_tweaker:temp potentials.temp.jockey set value " Jockey"
...
execute \
    if data storage spawner_tweaker:temp potentials.potentials[0] run function spawner_tweaker:spawner_tweaking/player/bossbars/potentials/basic_loop