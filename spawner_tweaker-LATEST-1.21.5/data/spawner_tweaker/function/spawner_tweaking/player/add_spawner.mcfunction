...
$execute \
    as @e[tag=st_glass,scores={spawner_tweaker_id=$(tweaker_id)}] run scoreboard players add spawners temp 1

execute \
    store result storage spawner_tweaker:temp name.spawners int 1 run scoreboard players get spawners temp
