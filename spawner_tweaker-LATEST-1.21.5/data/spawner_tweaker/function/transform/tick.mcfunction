...
##MAIN PROCESS
#Loop through the spawners to check \
    if loaded. If not, load. If loaded, prime and mark completed
...
function spawner_tweaker:transform/attempt_transform with storage spawner_tweaker:temp Priming[0]
...
scoreboard players add transform_timer st_priming 1
title @a actionbar [{"text":"Spawners transformed ","color":"gray"},{"score":{"name":"spawners_transformed","objective":"st_priming"},"color":"red"},{"text":"/"},{"score":{"name":"total_spawners","objective":"st_priming"},"color":"green"}]

#End when needed

execute \
    if score spawners_transformed st_priming >= total_spawners st_priming run function spawner_tweaker:transform/end
...
execute \
    if score transform_timer st_priming >= max temp run function spawner_tweaker:transform/end