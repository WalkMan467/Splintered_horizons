...
$execute \
    ...
    positioned $(x) $(y) $(z) run function spawner_tweaker:transform/transform with storage spawner_tweaker:temp Priming[$(n)]
$execute \
...
#Looping
$execute \
    if score n temp < $modification_limit spawner_tweaker run function spawner_tweaker:transform/attempt_transform with storage spawner_tweaker:temp Priming[$(next)]