...
$execute \
    if score remove temp matches 1.. run tellraw @s[tag=!st_transformer] [{"color":"red","text":"Removed "},{"color":"gray","text":"[ID:"},{"color":"gold","text":"$(id)"},{"color":"gray","text":"]"}," \
    as well \
    as all registered spawner locations under that id"]
$execute \
    if score remove temp matches 1.. run tellraw @s[tag=st_transformer] [{"color":"green","text":"Transformation was completed and "},{"color":"gray","text":"[ID:"},{"color":"gold","text":"$(id)"},{"color":"gray","text":"]"}," was removed"]

execute \
    unless score remove temp matches 1.. run tellraw @s [{"color":"dark_red","text":"That spawner id does not exist so it could not be removed."}]

execute \
    unless score remove temp matches 1.. run playsound block.note_block.didgeridoo master @s ~ ~ ~ 1 1
