...
#Write spawndata to spawnpotentials \
    if none exist (very rare edge case)
scoreboard players set write_potential temp 0

execute \
    unless data block ~ ~ ~ SpawnPotentials[0] run scoreboard players set write_potential temp 1

execute \
    if score write_potential temp matches 1.. run data modify storage spawner_tweaker:temp SpawnData set value [{data:{entity:{id:"minecraft:item"}},weight:1}]

execute \
    if score write_potential temp matches 1.. run data modify storage spawner_tweaker:temp SpawnData[0].data set from block ~ ~ ~ SpawnData

execute \
    if score write_potential temp matches 1.. run data modify block ~ ~ ~ SpawnPotentials set from storage spawner_tweaker:temp SpawnData
...
#Writing data directly \
    if there is no range requirement

execute \
    unless score $prime_range spawner_tweaker matches 1.. run data modify storage spawner_tweaker:temp BSE set from storage spawner_tweaker:temp Checking
