...
#Loop through the spawners to check \
    if loaded. If not, load. If loaded, update and mark completed
...
execute \
    if score spawners_updated st_priming >= total_spawners st_priming run function spawner_tweaker:bulk_edit/end
...
execute \
    if score bse_timer st_priming >= max temp run function spawner_tweaker:bulk_edit/end