...
$execute in $(dimension) \
    positioned $(x) $(y) $(z) \
    unless loaded ~ ~ ~ run data modify storage spawner_tweaker:temp Priming[$(n)].force_load set value 1b
$execute in $(dimension) \
    positioned $(x) $(y) $(z) \
...
#Looping
$execute \
    if score n temp <= total_spawners st_priming run function spawner_tweaker:spawner_priming/load_checker with storage spawner_tweaker:temp Priming[$(next)]