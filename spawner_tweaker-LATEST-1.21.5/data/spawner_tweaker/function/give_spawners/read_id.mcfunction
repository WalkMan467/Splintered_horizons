...
execute \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.attributes.[{id:"minecraft:movement_speed"}] run data modify storage spawner_tweaker:temp give.Speed_desc set value " 🏃 "
...
execute \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.attributes.[{id:"minecraft:scale"}] run data modify storage spawner_tweaker:temp give.Size_desc set value " ✥ "
...
execute \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.attributes.[{id:"minecraft:armor"}] run data modify storage spawner_tweaker:temp give.Armor_desc set value " ⛨ "
...
execute \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.Tags run data modify storage spawner_tweaker:temp give.Tag_prefix set value "Tags: "
...
execute \
    if data storage spawner_tweaker:temp give.tags[0] run scoreboard players set continue temp 1

execute \
    if score continue temp matches 1.. run function spawner_tweaker:give_spawners/concat with storage spawner_tweaker:temp variables

#Get the name (pain)
scoreboard players set name temp 0

execute \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.CustomName run scoreboard players set name temp 1

execute \
    if data storage spawner_tweaker:temp give.spawner.components."minecraft:custom_name" \
    unless data storage spawner_tweaker:temp give.spawner.components."minecraft:custom_data".spawner_tweaker_spawner run scoreboard players set name temp 2

execute \
    if data storage spawner_tweaker:temp give.spawner.components."minecraft:item_name" \
    unless data storage spawner_tweaker:temp give.spawner.components."minecraft:custom_data".spawner_tweaker_spawner run scoreboard players set name temp 2

execute \
    if score name temp matches 0 run data modify storage spawner_tweaker:temp give.name set string storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.id 10

execute \
    if score name temp matches 1 run data modify storage spawner_tweaker:temp give.name set from storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.CustomName

execute \
    if score name temp matches 2 \
    if data storage spawner_tweaker:temp give.spawner.components."minecraft:custom_name" run data modify storage spawner_tweaker:temp give.name set from storage spawner_tweaker:temp give.spawner.components."minecraft:custom_name"

execute \
    if score name temp matches 2 \
    if data storage spawner_tweaker:temp give.spawner.components."minecraft:item_name" run data modify storage spawner_tweaker:temp give.name set from storage spawner_tweaker:temp give.spawner.components."minecraft:item_name"

execute \
    if score name temp matches 0 \
    if data storage spawner_tweaker:temp give.spawner.SpawnPotentials[0].data.entity.Passengers[] run data modify storage spawner_tweaker:temp give.Jockey set value " Jockey"
...
execute \
    if score name temp matches 0 \
    if data storage spawner_tweaker:temp give.spawner run function spawner_tweaker:give_spawners/give_item_1 with storage spawner_tweaker:temp give

execute \
    if score name temp matches 1.. \
    if data storage spawner_tweaker:temp give.spawner run function spawner_tweaker:give_spawners/give_item_2 with storage spawner_tweaker:temp give

execute \
    unless data storage spawner_tweaker:temp give.spawner run tellraw @s[tag=st_give_single] {"color":"red","text":"This spawner id does not exist"}

execute \
    unless data storage spawner_tweaker:temp give.spawner run playsound minecraft:block.note_block.didgeridoo master @s[tag=st_give_single] ~ ~ ~ 1 1
