...
$execute \
    positioned ~ ~$(y) ~ \
    unless block ~ ~ ~ barrel run setblock ~ ~ ~ barrel[facing=up]{CustomName:'"Spawner Tweaker Spawners"'} replace
...
execute \
    if data storage spawner_tweaker:temp give.spawner run scoreboard players add spawners_generated temp 1

execute \
    if data storage spawner_tweaker:temp give.spawner run scoreboard players add slot_in_barrel temp 1

execute \
    if score slot_in_barrel temp matches 27.. run scoreboard players add give_all_y temp 1
...
execute \
    ...
    unless score give_all_id temp matches 500.. run function spawner_tweaker:give_spawners/give_all_loop with storage spawner_tweaker:temp give_all