#Sees \
    if the container here can be used in randomizer. If not, yeet it.
...
$execute \
    if score chest_pass temp matches 0 run data remove storage spawner_tweaker:temp Chests[{x:$(x),y:$(y),z:$(z),dimension:"$(dimension)"}]

execute \
    if score chest_pass temp matches 0 \
...
execute \
    if score chest_pass temp matches 0 align xyz \
...
$execute \
    if score chest_pass temp matches 1 run data modify storage spawner_tweaker:temp container_checker[$(n)].contents set from block ~ ~ ~
$execute \
    if score chest_pass temp matches 1 run data modify storage spawner_tweaker:temp container_randomizer append from storage spawner_tweaker:temp container_checker[$(n)]

execute \
    if score chest_pass temp matches 1 run particle minecraft:happy_villager ~ ~ ~ 0.5 0.5 0.5 0 10
