...
data modify entity @e[type=item,tag=wma.quit_item,tag=1,limit=1] Item set from entity @s ArmorItems[0]
summon item ~ ~1.5 ~ {Tags:[wma,wma.quit_item,"2",wma.need_some_help],Item:{id:"stone"},Age:-32767s,Glowing:1b}
data modify entity @e[type=item,tag=wma.quit_item,tag=2,limit=1] Item set from entity @s ArmorItems[1]
summon item ~ ~1.2 ~ {Tags:[wma,wma.quit_item,"3",wma.need_some_help],Item:{id:"stone"},Age:-32767s,Glowing:1b}
data modify entity @e[type=item,tag=wma.quit_item,tag=3,limit=1] Item set from entity @s ArmorItems[2]
summon item ~ ~0.9 ~ {Tags:[wma,wma.quit_item,"4",wma.need_some_help],Item:{id:"stone"},Age:-32767s,Glowing:1b}
data modify entity @e[type=item,tag=wma.quit_item,tag=4,limit=1] Item set from entity @s ArmorItems[3]
...
# rewear old helmet
data merge entity @s {ArmorItems:[{},{},{},{components:{"minecraft:attribute_modifiers":{modifiers:[{amount:0d,id:"wma.zero_items.3",slot:"head",type:"minecraft:armor",operation:"add_value"}]},"minecraft:unbreakable":{},"minecraft:custom_data":{wma:1}},id:"minecraft:leather_helmet"}]}
