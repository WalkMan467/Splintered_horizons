...
execute \
    as @e[type=marker,tag=cse.status_effect.weakness.data] \
    if score @s cse.status_effects.weakness.id = @e[type=!marker,tag=cse.status_effect.weakness.user,tag=cse.status_effect.weakness.data_target,limit=1] cse.status_effects.weakness.id run \
tag @s add cse.status_effect.weakness.data_target
$execute \
    as @n[type=marker,tag=cse.status_effect.weakness.data,tag=cse.status_effect.weakness.data_target] run \
function cse:status_effects/apply/weakness/marker_data/math {value:$(value), max:$(max)}
