...
execute \
    ...
    if score @s cse.status_effects.weakness.id = @e[type=!marker,tag=cse.status_effect.weakness.user,tag=cse.status_effect.weakness.data_target.merge,limit=1] cse.status_effects.weakness.id run \
tag @s add cse.status_effect.weakness.data_target.merge
