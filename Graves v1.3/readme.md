...
When a player dies they will leave a grave at the site of their death which will will keep their item and experience safe. Left or right click the grave to open it and drop its contents. This datapack is multiplayer-friendly and highly configurable, allowing you to make graves despawn, how long it takes for them to despawn, what item are dropped, and more!
...
### General Settings
**Despawn Despawn:** Graves will automatically despawn after a specific amount of time, causing them to drop their contents. Note that unlike item, this timer will continue when the grave is not loaded. (Disabled by default with a default time of 10 minutes)
...
**Keep Hotbar:** item in the hotbar are kept on death (disabled by default).

**Keep armor:** Equiped armor is kept on death (disabled by default).

**Keep Some item:** Some item are not dropped on death (disabled by default). The list of item defaults to weapons, armor, tools, and other equipment but can be changed in `m_graves/tags/item/kept_item`.

**Lose Some item:** Some item are permanently lost on death (disabled by default). The list of item is empty by default but can be changed in `m_graves/tags/item/lost_item`.

Some item tags have been made built-in for the convenience of editing the kept and lost item:
- `#m_graves:equipment` for weapons, armor, tools, and other equipment
- `#m_graves:mob_drops` for mob drops
- `#m_graves:rare_item` for item that have colored text when unenchanted (netherstars, dragon egg, etc)
