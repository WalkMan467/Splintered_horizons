...
execute store result score @n[tag=particle.tunder_red,tag=summon] duration run \
random value 20..59

$execute store result storage particle tunder_red.motion_x int 1 run \
random value -$(spread)..$(spread)
$execute store result storage particle tunder_red.motion_y int 1 run \
random value -$(spread)..$(spread)
$execute store result storage particle tunder_red.motion_z int 1 run \
random value -$(spread)..$(spread)

execute store result storage particle tunder_red.facing_x int 1 run \
random value 0..360

execute store result storage particle tunder_red.facing_y int 1 run \
random value -90..90
