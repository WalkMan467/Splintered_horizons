...
execute \
    if score #rdm global.main matches 0..75 run \
return 0
...
execute \
    as @e[tag=summon,tag=ink_painting_expansion.ink,type=item] at @s run \
function particle:ink_painting_expansion/ink/rdm_motion/use