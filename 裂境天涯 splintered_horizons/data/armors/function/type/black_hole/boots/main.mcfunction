...
execute unless score @s player.shift.skill.disable matches 1.. unless entity @s[gamemode=spectator] unless score @s player.animation.lock matches 1.. unless predicate players:detect/input/sneak if score @s armor.black_hole.boots.sneak matches 1..3 run function armors:type/black_hole/animation/boots/play
