...
execute \
    as @e[type=item_display,tag=aj.stellar_animation.root,tag=aj.stellar_animation.animation.boots_of_the_black_hole.playing,tag=!delete,distance=..10,limit=1] \
    if score @p[tag=armor.black_hole.animation.stop.user] player.id = @s armor.black_hole.boots.aj.id run \
tag @s add delete

execute \
    as @e[type=item_display,tag=aj.stellar_animation.camera,tag=aj.stellar_animation.camera.camera3,tag=!delete,distance=..10,limit=1] \
    if score @p[tag=armor.black_hole.animation.stop.user] player.id = @s armor.black_hole.boots.aj.id run \
tag @s add delete

execute \
    as @e[type=area_effect_cloud,tag=armor.black_hole.effect,tag=!delete,distance=..10,limit=1] \
    if score @p[tag=armor.black_hole.animation.stop.user] player.id = @s armor.black_hole.boots.aj.id run \
tag @s add delete
...
execute \
    as @n[type=item_display,distance=..10,tag=aj.stellar_animation.root,tag=aj.stellar_animation.animation.boots_of_the_black_hole.playing,tag=delete] at @s run \
