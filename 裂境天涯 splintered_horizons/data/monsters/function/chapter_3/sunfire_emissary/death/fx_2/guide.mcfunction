...
execute \
    if score #monsters.sunfire_emissary.x particle matches 360.. run \
scoreboard players set #monsters.sunfire_emissary.rot_y particle 1

execute \
    if score #monsters.sunfire_emissary.x particle matches 360.. run \
scoreboard players add #monsters.sunfire_emissary.y particle 15
...
execute \
    if score #monsters.sunfire_emissary.y particle matches ..90 \
    if score #monsters.sunfire_emissary.rot_y particle matches 0 rotated ~15 ~ run \
function monsters:chapter_3/sunfire_emissary/death/fx_2/guide

execute \
    if score #monsters.sunfire_emissary.y particle matches ..90 \
    if score #monsters.sunfire_emissary.rot_y particle matches 1 rotated ~15 ~15 run \
function monsters:chapter_3/sunfire_emissary/death/fx_2/guide
