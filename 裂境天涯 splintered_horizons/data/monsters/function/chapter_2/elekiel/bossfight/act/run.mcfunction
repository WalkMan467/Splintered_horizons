execute on target run tellraw @s {"translate": "story.chapter_2.bossfight.act.1","fallback": "--------------- BOSS 戰 ---------------"}
execute on target run tellraw @s {"translate": "story.chapter_2.bossfight.act.2","fallback": "BOSS: 「空之支配者」伊萊克爾"}
execute on target run tellraw @s "\n"
execute on target run tellraw @s {"translate": "monsters.chapter_2.bossfight.act.2","fallback": "來自其他舊世界的「空之支配者」伊萊克爾 ",italic:true,color:gray}
execute on target run tellraw @s {"translate": "monsters.chapter_2.bossfight.act.3","fallback": "除了擁有操控時間與空間的能力",italic:true,color:gray}
execute on target run tellraw @s {"translate": "monsters.chapter_2.bossfight.act.4","fallback": "因獲得 草之魔劍 - 翠綠茵草 力量獲得進一步提升",italic:true,color:gray}
execute on target run tellraw @s {"translate": "monsters.chapter_2.bossfight.act.5","fallback": "在戰鬥中會隨機使用 草之魔劍 力量與 它曾用來摧毀舊世界的 空之權能",italic:true,color:gray}
execute on target run tellraw @s "\n"
execute on target run tellraw @s "--------------------------------------"
