...
execute \
    unless entity @s[tag=monsters.elekiel_phase_2.portal.tp.player] run \
return 0
...
execute \
    as @e[type=block_display,tag=monsters.elekiel_phase_2.portal.tp] at @s \
    if score @s monster.elekiel_phase_2.skill.4.portal.id = @p[tag=monsters.elekiel_phase_2.portal.tp.player,tag=monsters.elekiel_phase_2.portal.tp.remove,gamemode=spectator] player.id run \
kill @s
