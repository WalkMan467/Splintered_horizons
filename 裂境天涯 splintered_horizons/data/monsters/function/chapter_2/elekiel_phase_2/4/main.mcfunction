...
execute as @p[sort=arbitrary,tag=monster.elekiel_phase_2.4.effect.target] at @s run tp @n[sort=arbitrary,tag=monster.elekiel_phase_2.4.effect.target.player_leave.detect,distance=..100,type=marker] ~ ~ ~

execute if score @s monster.elekiel_phase_2.skill.4.casting matches 1..100 unless entity @p[sort=arbitrary,tag=monster.elekiel_phase_2.4.effect.target] as @n[sort=arbitrary,tag=monster.elekiel_phase_2.4.effect.target.player_leave.detect,distance=..100,type=marker] at @s run function monsters:chapter_2/elekiel_phase_2/4/2
execute if score @s monster.elekiel_phase_2.skill.4.casting matches 100 if entity @p[sort=arbitrary,tag=monster.elekiel_phase_2.4.effect.target] as @p[tag=monster.elekiel_phase_2.4.effect.target] at @s run function monsters:chapter_2/elekiel_phase_2/4/2
