...
# 被動
execute if score @s monster.skill.cast.cd matches ..0 run function monsters:chapter_2/broken_moon/cast/cast
...
# 偵測是否沒有玩家在 60 格範圍內
execute unless entity @a[distance=..60,gamemode=!spectator,tag=!animation] run function monsters:chapter_2/broken_moon/void