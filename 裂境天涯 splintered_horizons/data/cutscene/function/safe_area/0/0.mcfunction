...
execute positioned 757 143 934 as @n[type=item_display,distance=..10,tag=aj.sophia.root] run function animated_java:sophia/remove/this
...
execute rotated 0.0 0.0 positioned 757.75 143 934 run function animated_java:sophia/summon {args: {animation: 'walk', start_animation: true}}
...
execute positioned 758 143 934 as @n[type=item_display,distance=..10,tag=aj.sophia.root] run tag @s add cutscene.safe_area.0.sophia
...
scoreboard players set @n[type=item_display,tag=cutscene.safe_area.0.sophia] cutscene.aj.animation.state 1
