...
execute \
    as @e[sort=arbitrary,tag=aj.stellar_animation.camera.camera3,tag=!temp,tag=aj.setup,type=item_display] at @s \
...
execute \
    ...
    if score @s weapon.earthquake_axe.user.id = @e[limit=1,sort=arbitrary,distance=..10,tag=temp,tag=aj.stellar_animation.camera.camera3,tag=aj.setup,type=item_display] weapon.earthquake_axe.user.id run \
tag @s add temp


execute \
    as @a[tag=temp,tag=weapon.earthquake_axe.user] at @s run \
spectate @e[sort=arbitrary,limit=1,distance=..10,tag=temp,tag=aj.stellar_animation.camera.camera3,tag=aj.setup,type=item_display]
