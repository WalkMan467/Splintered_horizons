# ===================================================
# 鐮 變形異獸 右鍵 階段 1 效果 / scythe morphing beast right click step 1 effect
...
execute \
    on attacker \
    if score @s player.health matches 4.. run \
damage @s 3 weapons:type/sword/nightfall

execute \
    on attacker \
    unless score @s player.health matches 4.. run \
return 0


execute \
    on attacker run \
function weapons:type/sword/nightfall/rc/1/calculate
