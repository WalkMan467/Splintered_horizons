...
execute \
    if score @s weapon.ruins_of_the_finality.effect.cd matches 1.. run \
return 0

execute \
    unless score @s weapon.ruins_of_the_finality.effect matches 5.. run \
scoreboard players add @s weapon.ruins_of_the_finality.effect 1
