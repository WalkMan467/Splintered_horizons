...
summon creeper ^ ^10 ^ {DeathLootTable:"",Silent:1b,Invulnerable:1b,PersistenceRequired:1b,NoAI:1b,Tags:["aj.stellar.camera_2","summon"],active_effects:[{id:"minecraft:invisibility",amplifier:255,duration:-1,show_particles:0b,show_icon:0b,ambient:0b}],attributes:[{id:"minecraft:scale",base:0.00001}]}
scoreboard players add #screen weapon.ruins_of_the_finality.id 1
scoreboard players operation @n[tag=aj.stellar.camera_2,tag=summon] weapon.ruins_of_the_finality.id = #screen weapon.ruins_of_the_finality.id
scoreboard players operation @s weapon.ruins_of_the_finality.id = #screen weapon.ruins_of_the_finality.id



execute \
    as @n[tag=summon,type=creeper] at @s facing entity @n[tag=aj.stellar.root] eyes run \
rotate @s ~180 0
...
spectate @n[tag=aj.stellar.camera_2,tag=summon]
title @s title {"text":"\uE002","font":"screen"}
title @s times 20 40 20

tag @n[tag=aj.stellar.camera_2,tag=summon] remove summon