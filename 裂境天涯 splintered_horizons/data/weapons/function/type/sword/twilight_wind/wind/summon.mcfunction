...
execute \
    as @n[tag=twilight_wind.wind,tag=summon,type=item_display] at @s \
    if score #rdm weapon.twilight_wind.timer matches 1 run \
rotate @s ~10 ~10

execute \
    as @n[tag=twilight_wind.wind,tag=summon,type=item_display] at @s \
    if score #rdm weapon.twilight_wind.timer matches 2 run \
rotate @s ~-10 ~10

execute \
    as @n[tag=twilight_wind.wind,tag=summon,type=item_display] at @s \
    if score #rdm weapon.twilight_wind.timer matches 3 run \
rotate @s ~10 ~-10

execute \
    as @n[tag=twilight_wind.wind,tag=summon,type=item_display] at @s \
    if score #rdm weapon.twilight_wind.timer matches 4 run \
rotate @s ~-10 ~-10
