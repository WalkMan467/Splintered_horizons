...
def generate_give_command(item):
    ...
    # passive
    if passive["is_passive_skills"]:
        lore.append(_title(item_data["id"], "passive_skills", passive["name"][1], passive["cd"]))
        translate_lines.append(f'"weapon.{item_data["id"]}.passive_skills" : "[{passive["name"][0]}]"')

        for i, line in enumerate(passive["info"], start=1):
            lore.append(_line(item_data["id"], "passive_skills", i, passive["name"][2]))
            translate_lines.append(f'"weapon.{item_data["id"]}.passive_skills.{i}" : "{line}"')

        lore.append({"text": ""})

    # ultimate
    if ultimate["is_ultimate"]:
        lore.append(_title(item_data["id"], "ultimate", ultimate["name"][1], ultimate["cd"]))
        translate_lines.append(f'"weapon.{item_data["id"]}.ultimate" : "[{ultimate["name"][0]}]"')

        for i, line in enumerate(ultimate["info"], start=1):
            lore.append(_line(item_data["id"], "ultimate", i, ultimate["name"][2]))
            translate_lines.append(f'"weapon.{item_data["id"]}.ultimate.{i}" : "{line}"')
    ...
    max_damage = f',max_damage={item_data["max_damage"]},damage=0'
    ...
    cmd = (
        ...
        f'lore={lore_json}'
        f'{attr_string},'
        f'max_stack_size={max_stack}{max_damage},'
