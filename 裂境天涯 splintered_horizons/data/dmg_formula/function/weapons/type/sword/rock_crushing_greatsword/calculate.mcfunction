...
execute store result score #temp atk run \
attribute @s minecraft:attack_damage get
scoreboard players operation @s atk *= #temp atk

# store & atk

execute store result storage temp values float 0.01 run \
scoreboard players get @s atk
function dmg_formula:weapons/type/sword/rock_crushing_greatsword/damage with storage temp
