## ---開頭--- ##
    ...
    execute \
    as @n[tag=world_area.main.safe_zone.melisna,type=mannequin] at @s \
...
rotate @s 125 0
    ...
    execute \
    as @n[tag=aj.isokla.root,tag=world_area.main.safe_zone.isokla,type=item_display] at @s \
    if entity @p[distance=..6] facing entity @p[distance=..6] eyes run \
rotate @s ~ 0
    execute \
    as @n[tag=aj.isokla.root,tag=world_area.main.safe_zone.isokla,type=item_display] at @s \
...
rotate @s 100 0
    ...
    # Detect
    execute \
    positioned 774 146 987 store result score #world_area.main.safe_zone.melisna global.main \
