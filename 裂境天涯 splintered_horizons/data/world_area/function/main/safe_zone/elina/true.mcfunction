...
execute \
    as @n[tag=aj.elina.root,limit=1] run \
...
function animated_java:elina/summon {args: {animation: 'idle', start_animation: true}}


execute \
    positioned 741 145 968 run \
tag @n[tag=aj.elina.root,distance=..2] add world_area.main.safe_zone.elina
