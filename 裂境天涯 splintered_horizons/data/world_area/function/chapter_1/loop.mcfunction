...
advancement grant @s only world_area:chapter_1/abyssal_ruins/out
    ...
    execute \
    if entity @s[gamemode=!spectator] \
    if biome ~ ~ ~ world_area:chapter_1/realm_of_the_black_hole run \
advancement grant @s only world_area:chapter_1/realm_of_the_black_hole/in
    execute \
    unless biome ~ ~ ~ world_area:chapter_1/realm_of_the_black_hole run \
advancement grant @s only world_area:chapter_1/realm_of_the_black_hole/out
