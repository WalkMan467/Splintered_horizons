...
execute store result score #world_area.realm_of_the_black_hole.fx global.main run \
random value 1..2


execute \
    if score #world_area.realm_of_the_black_hole.fx global.main matches 1 run \
scoreboard players set @e[type=area_effect_cloud,tag=world_area.realm_of_the_black_hole.fx,tag=summon] world_area.realm_of_the_black_hole.fx 10

execute \
    if score #world_area.realm_of_the_black_hole.fx global.main matches 2 run \
scoreboard players set @e[type=area_effect_cloud,tag=world_area.realm_of_the_black_hole.fx,tag=summon] world_area.realm_of_the_black_hole.fx -10
