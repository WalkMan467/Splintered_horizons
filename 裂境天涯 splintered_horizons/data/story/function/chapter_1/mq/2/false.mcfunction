scoreboard players set #story.chapter_1.mq.2_temp global.main 0
scoreboard players set story.chapter_1.mq.2 story.chapter_1 1

# Change the Weather \
{
    execute \
    if score #weather.temp global.main matches 0 run \
    weather clear

    execute \
    if score #weather.temp global.main matches 1 run \
    weather rain

    execute \
    if score #weather.temp global.main matches 2 run \
    weather thunder
#\
}
