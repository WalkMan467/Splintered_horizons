...
execute on target run \
tellraw @s [{"translate": "sys.walkman467.1", "fallback": "哈囉 %s 我是 WalkMan467", "with": [{"selector": "@s","color": "green","bold": true}],"color": "white","bold": false}]

execute on target run \
tellraw @s [{"translate": "sys.walkman467.2", "fallback": "夢想成為 Unreal 5、Unity 全職獨立遊戲設計師的技術宅","color": "white"}]

execute on target run \
tellraw @s [{"translate": "sys.walkman467.3", "fallback": "如果你喜歡這張地圖的話可以點擊下方連結支持我","color": "white"}]

execute on target run \
tellraw @s [{"translate": "sys.walkman467.4", "fallback": "這會給我很大的創作動力","color": "white"}]
...
execute on target run \
tellraw @s [{"translate": "sys.walkman467.6", "fallback": "若你想知道更多地圖的資訊以及設計緣由的話可以查看關於這張地圖的 WIKI","color": "white"}]
