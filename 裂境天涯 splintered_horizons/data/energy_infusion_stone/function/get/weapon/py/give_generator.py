...
def generate_give_command(item):
    ...
    stored_enchantments = item_data.get("stored_enchantments", [])
    ...
    stored_enchantments_str = ""
    if stored_enchantments and stored_enchantments != [""]:
        parsed_enchantments = []
        for entry in stored_enchantments:
    ...
    # ===== give =====
    cmd = (
        f'give @s enchanted_book['
        f'item_name={json.dumps([{"translate": f"item.{item_id}", "color": name[1], "bold": True}], ensure_ascii=False)},'
        f'lore={lore_json},'
        f'item_model={item_model}'
    )

    if stored_enchantments_str:
        cmd += f',{stored_enchantments_str}'
    ...
    lang_block += "# ==============================\n"
    lang_block += f'# "item.{item_id}" : "{name[0]}",\n'
