...
execute \
    ...
    if score @s player.id = @n[distance=0..,sort=arbitrary,tag=armor.shockwave.display,tag=armor.shockwave.display.tp,type=item_display] armor.enchantment.shockwave.id run \
tag @s add armor.shockwave.display.tp
