...
execute if entity @n[sort=arbitrary,distance=..1.05,tag=monsters.segrina,type=zombie] run \
    return run \
function bossfight:chapter_2/segrina/skills/2/raycast/final
...
execute \
    ...
    positioned ^ ^ ^0.25 run \
    return run \
function bossfight:chapter_2/segrina/skills/2/raycast/detect

execute \
    unless score #monster.segrina.skill.2.raycast monster.segrina.skill.2.casting matches ..49 \
    positioned ^ ^ ^0.25 run \
schedule function bossfight:chapter_2/segrina/skills/2/raycast/broken 1t