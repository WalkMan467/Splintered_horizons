
scoreboard players add @s mob.duration 1

# digit
function bossfight:chapter_2/elekiel_phase_2/skills/4/portal/digit/use

rotate @s ~ 0


# detect

execute \
    as @a[distance=..5] \
    positioned ~-2 ~ ~-2 \
    if entity @s[dx=3,dy=1,dz=3] \
    unless score @s player.animation.lock matches 1.. run \
advancement grant @s only bossfight:chapter_2/elekiel_phase_2/skills/2/in

execute \
    as @a[distance=..5] \
    positioned ~-2 ~ ~-2 \
    unless entity @s[dx=3,dy=1,dz=3] run \
advancement grant @s only bossfight:chapter_2/elekiel_phase_2/skills/2/out


# silhouette
scoreboard players operation #duration mob.main = @s mob.duration
scoreboard players operation #duration mob.main %= #5 mob.main

execute \
    if score #duration mob.main matches 0 run \
function bossfight:chapter_2/elekiel_phase_2/skills/4/portal/crack_silhouette/use


execute \
    if score @s mob.duration matches 500.. run \
kill @s