...
execute \
    store result score #scale mob.main run \
random value 20..80

execute \
    store result storage mob:temp temp.scale[1] float 0.1 run \
scoreboard players get #scale mob.main

execute \
    store result storage mob:temp temp.translation[1] float -0.006 run \
scoreboard players get #scale mob.main
...
execute \
    store result score #x1 mob.main run \
data get entity @s Pos[0] 10

execute \
    store result score #y1 mob.main run \
data get entity @s Pos[1] 10

execute \
    store result score #z1 mob.main run \
data get entity @s Pos[2] 10


execute \
    store result score #x2 mob.main run \
random value -20..20

execute \
    store result score #y2 mob.main run \
random value -10..-5

execute \
    store result score #z2 mob.main run \
random value -20..20


execute \
    store result entity @s Pos[0] double 0.1 run \
scoreboard players operation #x1 mob.main += #x2 mob.main

execute \
    store result entity @s Pos[1] double 0.1 run \
scoreboard players operation #y1 mob.main += #y2 mob.main

execute \
    store result entity @s Pos[2] double 0.1 run \
